...
function LFGListHookModule.SetupModifiedEntryFrames()
    ...
    function EntryModMixin:OnUpdate()
        ...
        self.NewPlayerFriendlyIcon:SetPoint("RIGHT", self.DataDisplay.RoleCount.TankCount, "LEFT", -2, 0)
    ...
    function EntryModMixin:Reset()
        ...
        for regionName, anchors in pairs(EntryFrameChildRegionBlizzAnchors) do
            ...
            for _, anchor in ipairs(anchors) do
                assert(anchor.point, "Anchor point not found")
                assert(anchor.relativePoint, "Anchor relativePoint not found")
                local relativeTo = (anchor.relativeTo and self[anchor.relativeTo]) or self
                assert((not anchor.relativeTo or relativeTo),
                    "Anchor relativeTo region not found", anchor, self
                )
    ...
    local ROLE_COUNT_REGION_PARENT_KEYS = {"TankCount", "HealerCount", "DamagerCount"};
    ...
    function DataDisplayModMixin:Init()
        ...
        -- reduce spacing between counts and role icons in the RoleCount data display.
        for _, key in ipairs(ROLE_COUNT_REGION_PARENT_KEYS) do
            local fontString = self.RoleCount[key]
            assert(fontString, "RoleCount child region not found: ", key, self.RoleCount)
            fontString:SetWidth(ROLE_COUNT_DEFAULT_WIDTH * 0.65)
    ...
    function DataDisplayModMixin:Reset()
        if not self.CustomEnumerate then return end;
        for _, fontStringKey in ipairs({"TankCount", "HealerCount", "DamagerCount"}) do
            local fontString = self.RoleCount[fontStringKey]
            assert(fontString, "RoleCount child region not found: ", fontStringKey, self.RoleCount)
            fontString:SetJustifyH("CENTER")
            fontString:SetWidth(ROLE_COUNT_DEFAULT_WIDTH)
        end
